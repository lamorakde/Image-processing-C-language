...
ie=(int *)malloc(dimx*dimy*sizeof(int));
id=(int *)malloc(dimx*dimy*sizeof(int));
  ...
  for(j=1;j<dimx-1;j++)
{
    ...
    id[i*dimx+j]=a[0];            //取最大，做膨胀运算
    ie[i*dimx+j]=a[8];            //取最小，做腐蚀运算
    ir[i*dimx+j]=id[i*dimx+j]-ie[i*dimx+j];       // 膨胀减去腐蚀得到边缘 //
}
