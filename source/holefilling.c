...
{
    unsigned char chi;
    int a,test,jump,circle;
    int x,y;
    long int *io,*ir,*ic,*it,*it2;
...
for(i=0;i<dimy;i++)
 for(j=0;j<dimx;j++)
 {
     ic[i*dimx+j]=255-io[i*dimx+j];                //  complementaire de l`image
 }
...
for(i=1;i<dimy-1;i++)
    ...
    for(i=1;i<dimy-1;i++)
     for(j=1;j<dimx-1;j++)
     {
         if((it2[i*dimx+j]==255)&&(ic[i*dimx+j]==255))
         {
            it[i*dimx+j]=255;
         }
     }
...
for(i=1;i<dimy-1;i++)
 for(j=1;j<dimx-1;j++)
 {
     if((it[i*dimx+j]==255)||(io[i*dimx+j]==255))
     {
         ir[i*dimx+j]=255;
     }
     else ir[i*dimx+j]=0;
